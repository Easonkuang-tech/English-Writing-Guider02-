...
- 用户登录支持 Email Magic Link 和邮箱 + 密码。
- 未登录用户不能访问主应用。
- 新用户默认角色为 `user`。
- 管理员角色为 `admin`，只能通过受控 SQL 或后台授权。
- `/admin` 仅允许管理员访问。
- 管理员可以查看用户列表、注册时间、最近登录、写作数、语料数、阶段分布、复习任务和 AI 失败统计。
- 管理员可以停用、启用或重新发送登录邮件。
- 用户私人作文默认只显示元数据；查看全文必须由管理员主动打开，并保留审计记录。
- RLS 是权限边界，前端隐藏按钮不能代替数据库权限。
